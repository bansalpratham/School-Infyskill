      ...
      <div className="space-y-3">
        {loading ? (
          ...
          items.map((notification, index) => (
            ...
            >
              <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0", "bg-info/10")}>
                <Bell className={cn("w-6 h-6", "text-info")} />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <h4 className={cn("font-semibold", !notification.isRead && "text-primary")}>{notification.title}</h4>
                  {!notification.isRead && <span className="w-2.5 h-2.5 bg-primary rounded-full flex-shrink-0 mt-1.5" />}
                </div>
