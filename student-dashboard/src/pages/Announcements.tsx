    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Button
          ...
          disabled={markingAll || loading || !!error || unreadCount === 0}
        >
          ...
          items.map((a, index) => (
            <Card
              ...
              className={cn(
                ...
                !a.isRead && "border-l-4 border-l-primary bg-primary/5"
              )}
            ...
            >
              <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center flex-shrink-0", "bg-warning/10")}>
                <Megaphone className={cn("w-6 h-6", "text-warning")} />
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <h4 className={cn("font-semibold", !a.isRead && "text-primary")}>{a.title}</h4>
                  {!a.isRead && <span className="w-2.5 h-2.5 bg-primary rounded-full flex-shrink-0 mt-1.5" />}
                </div>
